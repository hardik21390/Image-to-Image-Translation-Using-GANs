...
class cityscapes:
    def __init__(self, data_path, result_path):
        ...
        self.color2ID = {label.color: label.id for label in labels.labels}  # dictionary mapping train IDs to colors as 3-tuples
    ...
    def load_label(self, idx):
        ...
        """
        label_mask = Image.open('{}/{}_A_real_B.png'.format(self.res, idx))
        label_mask = np.asarray(label_mask)
        label = np.zeros(
            (label_mask.shape[0], label_mask.shape[1]), dtype=np.uint8
        )
        for color, label_value in self.color2ID.items():
            label[np.all(label_mask == color, axis=-1)] = label_value
    ...
    def list_label_frames(self):
        ...
        files = sorted(glob.glob('{}/*_A_fake_A.png'.format(self.res)))
        frames = [file2idx(f) for f in files]
        return frames
